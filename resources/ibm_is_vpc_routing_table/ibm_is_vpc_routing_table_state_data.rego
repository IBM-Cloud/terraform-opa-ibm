package ibmcloud.resources.state.data.ibm_is_vpc_routing_table
...
resources_map[attr]{
    attr := state.get_resources("ibm_is_vpc_routing_table", "data").resources[_]
}
...
accept_routes_from = ret {
    ret := {concat(".", [res.type, res.name]): object.get(res.attributes, "accept_routes_from", null) |
        res := resources_map[_]
        true
     }
}
