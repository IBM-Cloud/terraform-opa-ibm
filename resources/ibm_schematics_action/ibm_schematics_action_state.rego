package ibmcloud.resources.state.ibm_schematics_action
...
resources_map[attr]{
    attr := state.get_resources("ibm_schematics_action", "managed").resources[_]
}
...
inventory = ret {
    ret := {concat(".", [res.type, res.name]): object.get(res.attributes, "inventory", null) |
        res := resources_map[_]
        true
     }
}
...
bastion_credential = ret {
    ret := {concat(".", [res.type, res.name]): object.get(res.attributes, "bastion_credential", null) |
        res := resources_map[_]
        true
     }
}
