package ibmcloud.resources.state.ibm_is_floating_ip
...
resources_map[attr]{
    attr := state.get_resources("ibm_is_floating_ip", "managed").resources[_]
}
status = ret {
...
target_list = ret {
    ret := {concat(".", [res.type, res.name]): object.get(res.attributes, "target_list", null) |
        res := resources_map[_]
        true
     }
}
