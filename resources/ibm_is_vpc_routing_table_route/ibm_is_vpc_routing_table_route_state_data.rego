package ibmcloud.resources.state.data.ibm_is_vpc_routing_table_route
...
resources_map[attr]{
    attr := state.get_resources("ibm_is_vpc_routing_table_route", "data").resources[_]
}
vpc = ret {
...
creator = ret {
    ret := {concat(".", [res.type, res.name]): object.get(res.attributes, "creator", null) |
        res := resources_map[_]
        true
     }
}
